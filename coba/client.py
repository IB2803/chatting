...
class BubbleMessage(QLabel):
    def __init__(self, text, is_me, sender_name, time, parent=None):
        ...
        # Set size policy to allow wrapping
        self.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.MinimumExpanding)
        
        # Create message bubble styling similar to WhatsApp
        if is_me:
            message_html = f"""
            <div style='font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; 
                       font-size: 15px; line-height: 1.4; word-wrap: break-word; overflow-wrap: break-word;'>
                <div style='color: #FFFFFF; margin-bottom: 8px; white-space: pre-wrap;'>{text}</div>
            ...
            <div style='font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; 
                       font-size: 15px; line-height: 1.4; word-wrap: break-word; overflow-wrap: break-word;'>
                <div style='color: #2C3E50; margin-bottom: 8px; white-space: pre-wrap;'>{text}</div>
    ...
    def sizeHint(self):
        # Override sizeHint to allow proper height calculation
        size = super().sizeHint()
        # Ensure minimum width but allow height to expand
...
class ChatWindow(QWidget):
    ...
    def setup_ui(self):
        ...
        # Message input area
        input_area = QWidget()
        input_area.setFixedHeight(100)  # Tinggi diperbesar untuk multi-line
        input_area.setStyleSheet("""
            QWidget {
                background-color: #FFFFFF;
                border-top: 1px solid #E8ECEF;
        ...
        # Message input - tinggi diperbesar dan allow multi-line
        self.message_input = QTextEdit()
        self.message_input.setMaximumHeight(70)
        self.message_input.setMinimumHeight(40)
        self.message_input.setPlaceholderText("Type a message... (Press Ctrl+Enter to send)")
        self.message_input.setStyleSheet("""
            QTextEdit {
                background-color: #F1F3F4;
                border: none;
    ...
    def add_message_to_ui(self, message_data):
        ...
        is_me = message_data['sender_id'] == self.user['id']
        bubble = BubbleMessage(
